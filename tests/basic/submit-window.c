...
void test1()
{
	...
	// Check 1
	test_check_waitfor(&test, atomic_load(&finished_tasks) == 1, 1000, "Could submit the task (Default)");
}
...
void test2()
{
	...
	// Check 0
	test_check_waitfor(&test, atomic_load(&finished_tasks) == 0, 10, "Added %d tasks to the submit window only", NTASKS - 1);
	// Submit last
	CHECK(nosv_create(&tasks[NTASKS - 1], task_type, 0, NOSV_CREATE_NONE));
	CHECK(nosv_submit(tasks[NTASKS - 1], NOSV_SUBMIT_NONE));
	// Check NTASKS
	test_check_waitfor(&test, atomic_load(&finished_tasks) == NTASKS, 1000, "Flushed the submit window after submitting one more");
}
...
void test3()
{
	...
	// Check NTASKS-1
	test_check_waitfor(&test, atomic_load(&finished_tasks) == NTASKS - 1, 1000, "Flushed the submit window explicitly");
}
...
void test5()
{
	...
	// Check 0
	test_check_waitfor(&test, atomic_load(&finished_tasks) == 0, 10, "Added %d tasks to the submit window only", NTASKS / 2);
	...
	CHECK(nosv_submit(tasks[NTASKS - 1], NOSV_SUBMIT_NONE));
	// Check NTASKS
	test_check_waitfor(&test, atomic_load(&finished_tasks) == (NTASKS / 2 + 1), 1000, "Flushed the submit window after submitting one task");
}
