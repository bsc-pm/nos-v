...
int main()
{
	...
	nosv_create(&tasks[0], task_type, 0, NOSV_CREATE_NONE);
	clock_gettime(CLOCK_MONOTONIC, &start);
	nosv_submit(tasks[0], NOSV_SUBMIT_DEADLINE_WAKE);
	nosv_submit(tasks[0], NOSV_SUBMIT_NONE);
	if (wait_for_tasks(1, 5, start.tv_sec)) {
		test_check(&test, 0, "Waiting for a waitfor task that was woken up before it was scheduled");
		return 1;
	}
	nosv_destroy(tasks[0], NOSV_DESTROY_NONE);
	...
	for (int t = 0; t < NTASKS; ++t)
		nosv_submit(tasks[t], NOSV_SUBMIT_DEADLINE_WAKE);
