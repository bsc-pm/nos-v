...
int main() {
	...
	int res = nosv_type_init(&ext, NULL, NULL, NULL, NULL, NULL, NOSV_TYPE_INIT_EXTERNAL, NULL);
	assert(!res);
	...
	res = nosv_type_init(&worker, worker_run, NULL, worker_comp, "worker", NULL, NOSV_TYPE_INIT_NONE, NULL);
	assert(!res);
