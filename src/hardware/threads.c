...
static inline void worker_wake_internal(nosv_worker_t *worker, cpu_t *cpu)
{
	...
	if (worker->cpu != cpu) {
		instr_affinity_remote(cpu->logical_id, worker->tid);
...
static void killer_task_run_callback(nosv_task_t task)
{
	...
	atomic_store_explicit(&current_process_manager->leader_shutdown_cpu, current_worker->cpu->logical_id, memory_order_relaxed);
...
static inline void worker_coordinate_shutdown(void)
{
	...
	char leader_shutdown_cpu = current_worker->cpu->logical_id == leader_id;
...
static inline void *worker_start_routine(void *arg)
{
	...
	assert(current_worker);
	assert(current_worker->cpu);
	cpu_set_current(current_worker->cpu->logical_id);
	...
	instr_thread_init();
	instr_thread_execute(current_worker->cpu->logical_id, current_worker->creator_tid, (uint64_t) arg);
	instr_worker_enter();
	instr_kernel_init(&kinstr);
...
int worker_yield_if_needed(nosv_task_t current_task)
{
	...
	task_execution_handle_t handle = scheduler_get(cpu->logical_id, SCHED_GET_NONBLOCKING);
...
void worker_block(void)
{
	...
	assert(cpu);
	cpu_set_current(cpu->logical_id);
...
nosv_worker_t *worker_create_local(thread_manager_t *threadmanager, cpu_t *cpu, task_execution_handle_t handle)
{
	...
	// both threads.
	instr_thread_create(cpu->logical_id, (uint64_t) worker);
